...
msg -bar
    ...
    if [[ $conect = "PPriv" ]]; then
        ...
        echo "master=ChuKK-SCRIPT" > ${ADM_tmp}/pwd.pwd
...
[[ -z ${texto_soket} ]] && texto_soket='<span style="color: #ff0000;"><strong><span style="color: #ff9900;">By</span>-<span style="color: #008000;">@ChuKK-SCRIPT</span>-ADM</strong></span>'
...
echo -e "[Unit]
Description=$1 Parametizado Service by @ChuKK-SCRIPT
...
WantedBy=multi-user.target" > /etc/systemd/system/python.$porta_socket.service
    ...
    if [[ $conect = "PGet" ]]; then
        [[ "$(ps x | grep "PGet.py" | grep -v "grep" | awk -F "pts" '{print $1}')" ]] && {
            ...
            print_center -azu   "Su Contraseña Gettunel es: $(msg -ama "ChuKK-SCRIPT")"
            msg -bar
        } || {
...
def main(host=LISTENING_ADDR, port=LISTENING_PORT):
    ...
    print "\033[0;34m━"*10,"\033[1;32m ChuKK-SCRIPT ADM - LITE","\033[0;34m━\033[1;37m"*11,"\n"
...
#-----------SELECCION------------
selecPython () {
echo -e "\e[91m\e[43m  ==== SCRIPT MOD ChuKK-SCRIPT|EDICION ====  \033[0m \033[0;33m[$(less ${ADM_inst}/v-local.log)]"
msg -bar
...
case ${selection} in
    1)
    wget -q -O /etc/adm-lite/PDirect.py https://raw.githubusercontent.com/ChuKK-SCRIPT/ChuKK-SCRIPT-Script/master/Python/PDirect.py
    mod1 "${conect}" 
    sleep 2s
    ;;
    2)
